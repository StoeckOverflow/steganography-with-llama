...
class ArithmeticProbOrdHider:
    ...
    def __init__(self, llm: Llama, disable_tqdm: bool = True):
        self.llm = llm
        self.get_valid_tokens = self.initialize_token_getter(self.llm)
    ...
    def initialize_token_getter(llm: Llama) -> Callable[[str, int, bool], list[str]]:
        def _get_valid_token(prompt: str, bits_per_token: int = 3, get_end_condition: bool = False, recursive_extra_tokens: int = 0) -> list[str]:
            ...
            for token in next_token_probs:
                ...
                return _get_valid_token(prompt, bits_per_token, get_end_condition, recursive_extra_tokens + 1)[:2**bits_per_token]
    ...
    def hide_in_single_article(self, binary_secrets: list[str], prompt: str, bits_per_token: int = 3, soft_max_chars_limit: int = 450) -> tuple[str, str]:
        """
        ...
        for j, binary_secret in tqdm.tqdm(enumerate(binary_secrets), "Hidding message nr: ", disable=self.disable_tqdm):
            ...
            for i, next_bits in tqdm.tqdm([(_i, binary_secret[_i:_i+bits_per_token]) for _i in range(0, len(binary_secret), bits_per_token)], "Message hidden: "):
                ...
                doctored_article = doctored_article + next_token
                if len(doctored_article) > soft_max_chars_limit and doctored_article.endswith((".", "!", "?")):
                    return doctored_article, [binary_secret[i+bits_per_token:], *binary_secrets[j+1:]]

            # Append one "out-of-range" token to signalize the change from one secret to the next
            doctored_article += self.get_valid_tokens(doctored_article, bits_per_token=bits_per_token, get_end_condition=True)
        
        # Once all secrets are hidden, append another "out-of-range" token to signalize ALL secrets are hidden
        doctored_article += self.get_valid_tokens(doctored_article, bits_per_token=bits_per_token, get_end_condition=True)
        return doctored_article, []
    
    def hide_in_whole_newsfeed(self, news_feed: list[str], binary_secrets: list[str], bits_per_token: int = 3, soft_max_chars_lim: int = 450, nr_prompt_words: int = 5) -> dict[str, list[str]]:
        """
        ...
        for news_article in tqdm.tqdm(news_feed, "Hidding messages is article nr: ", disable=self.disable_tqdm):
            ...
            doctored_article, remaining_secrets = self.hide_in_single_article(remaining_secrets, prompt, bits_per_token, soft_max_chars_lim)
    ...
    def get_next_possible_token(next_token_probs: list[str], news_article: int, i: int) -> Union[str, bool]:
            ...
            next_possible_tokens = [token for token in next_token_probs if token in news_article[i:i+max_len_next_token]] # If there's still more than one that would fit
    ...
    def retrieve_single_secret_from_single_article(self, news_article: str, analyzed_feed: str, bits_per_token: int = 3) -> tuple[str, bool]:
        decoded_msg = ""
        ...
        while (i := len(analyzed_feed)) < len(news_article):
            next_token_probs = self.get_valid_tokens(analyzed_feed, bits_per_token=bits_per_token)
            ...
            if next_token is False:
                ...
                temp_feed += self.get_valid_tokens(temp_feed, bits_per_token, True)
                next_token_probs = self.get_valid_tokens(temp_feed, bits_per_token=bits_per_token)
            ...
            analyzed_feed += next_token
            decoded_msg += self.pad_with_0(bin(chosen_ind)[2:], bits_per_token)
            pbar.n = i
            pbar.refresh()
    ...
    def retrieve_multiple_secrets_from_single_article(self, news_article: str, nr_prompt_words: int = 5, bits_per_token: int = 3) -> tuple[str, bool]:
        analyzed_feed = " ".join(self.split_in_separators(news_article)[:nr_prompt_words])
        decoded_messages = []
        all_messages_found = False
        while True:
            decoded_msg, all_messages_found, analyzed_feed = self.retrieve_single_secret_from_single_article(news_article, analyzed_feed, bits_per_token)
    ...
    def retrieve_multiple_secrets_from_news_feed(self, news_feed: list[str], nr_prompt_words: int = 5, bits_per_token: int = 3) -> list[str]:
        decoded_secrets = []
        fully_decoded_flags = []
        for news_article in tqdm.tqdm(news_feed, "Retrieving secret from article nr: ", disable=self.disable_tqdm):
            decoded_msgs, completely_decoded_flags = self.retrieve_multiple_secrets_from_single_article(news_article, nr_prompt_words, bits_per_token)
