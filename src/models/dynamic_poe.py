...
class DynamicPOE:
    ...
    def hide(self, message: str, news_feed: dict[str, list[str]], try_extra_compression: bool = True) -> dict[str, list[str]]:
    ...
    def recover(self, doctored_news_feed: dict[str, list[str]], vocabulary: Iterable = None) -> str:
        if vocabulary is None:
            vocabulary = self.get_default_vocabulary()
        decoded_binary_messages = self.hider.retrieve_multiple_secrets_from_news_feed(doctored_news_feed["feed"])
