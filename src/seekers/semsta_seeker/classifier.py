...
class Classifier_Trainer():
    ...
    def train_classifier(self, original_encoder_features, fused_features, train_labels, num_epochs=100, learning_rate=0.0001, batch_size=64):
    ...
    def train_classifier_with_cross_validation(self, original_encoder_features, fused_features, train_labels, num_epochs=100, learning_rate=0.0001, batch_size=64, k_folds=10):        
        ...
        for fold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(y_numpy)), y_numpy)):
            ...
            # Re-initialize model here if necessary to reset weights for each fold
            ...
            for epoch in range(num_epochs):
                ...
                #print(f'Fold {fold+1}, Epoch [{epoch+1}/{num_epochs}], Training Loss: {total_loss/len(train_loader)}')
