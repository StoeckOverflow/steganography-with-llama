...
class SemanticFeatureExtractor(nn.Module):
    def __init__(self, output_size, bert_model_name='bert-base-uncased'):
...
class SemanticFeatureExtractor(nn.Module):
    ...
    '''
    def forward(self, newsfeed):
        inputs = self.bert_tokenizer(newsfeed, padding=True, truncation=False, return_tensors="pt")
        input_ids = inputs['input_ids']
        attention_mask = inputs['attention_mask']
        outputs = self.bert_model(input_ids=input_ids, attention_mask=attention_mask)
        last_layer_output = outputs.last_hidden_state  # Shape: (batch_size, seq_length, hidden_size)
        dense_output = self.dense(last_layer_output)
        confidence_scores = self.sigmoid(dense_output)
        averaged_confidence_scores = confidence_scores.mean(dim=1)
        return confidence_scores
    '''